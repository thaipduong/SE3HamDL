...
class RnHamNODEdV(torch.nn.Module):
    '''
    ...
    def __init__(self, M_net = None, dV_net = None, g_net = None, device=None, x_dim = 1, u_dim=3, init_gain=1):
        super(RnHamNODEdV, self).__init__()
        ...
        if M_net is None:
            ...
            self.M_net = M_net
        if dV_net is None:
            self.dV_net = MLP(self.theta_dim, 500, 1, init_gain=init_gain).to(device)
        else:
            self.dV_net = dV_net
