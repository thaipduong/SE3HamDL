...
from se3hamneuralode import SE3FAHamNODE_2D_BigDrone
...
def get_args():
    ...
    parser.add_argument('--name', default='fadronesim', type=str, help='only one option right now')
...
def train(args):
    ...
    #path = '{}/run{}/fadronesim-se3ham-rk4-5p-final-5000.tar'.format(args.save_dir, run)
    #model.load_state_dict(torch.load(path, map_location=device))
    # Save/load pre-init model
    #path = '{}/fadronesim-se3ham-rk4-5p-pre-init.tar'.format(args.save_dir)
    #model.load_state_dict(torch.load(path, map_location=device))
    ...
    # Collect data
    data = get_dataset(test_split=0.9, save_dir=args.save_dir)
    ...
    for step in range(0,args.total_steps + 1):
        ...
        if step % args.print_every == 0:
            ...
            label = '-se3ham'
            path = '{}/run{}/{}{}-{}-{}p-{}.tar'.format(args.save_dir, run, args.name, label, args.solver, args.num_points, step+1)
            torch.save(model.state_dict(), path)
        # Gradient descent
        t = time.time()
        if step > 0:
        ...
        if step % args.print_every == 0:
            ...
            # os.makedirs(args.save_dir) if not os.path.exists(args.save_dir) else None
            # label = '-se3ham'
            # path = '{}/run{}/{}{}-{}-{}p3-{}.tar'.format(args.save_dir, run, args.name, label, args.solver, args.num_points, step+1)
            # torch.save(model.state_dict(), path)
...
if __name__ == "__main__":
    ...
    os.makedirs(args.save_dir) if not os.path.exists(args.save_dir) else None
    label = '-se3ham'
    path = '{}/run{}/{}{}-{}-{}p-final-{}.tar'.format(args.save_dir, run, args.name, label, args.solver, args.num_points, args.total_steps)
    torch.save(model.state_dict(), path)
    path = '{}/run{}/{}{}-{}-{}p-stats-final-{}.pkl'.format(args.save_dir, run, args.name, label, args.solver, args.num_points, args.total_steps)
