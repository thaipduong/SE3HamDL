...
from se3hamneuralode import L2_loss, from_pickle, MLP, PSD, SE3FAHamNODE_2D_BigDrone
...
def get_model():
    model =  SE3FAHamNODE_2D_BigDrone(device=device, pretrain = False).to(device)
    path = 'data/run6/fadronesim-se3ham-rk4-2p-final-5000.tar'
    model.load_state_dict(torch.load(path, map_location=device))
    path = 'data/run6/fadronesim-se3ham-rk4-2p-stats-final-5000.pkl'
...
if __name__ == "__main__":
    ...
    # Plot g_v(q)
    fig = plt.figure(figsize=figsize)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 0, 0], 'r--', linewidth=line_width,
             label=r'$g_{v}(q)[0,0]$')
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 1, 1], 'g--', linewidth=line_width,
             label=r'$g_{v}(q)[1,1]$')
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 2, 2], 'b--', linewidth=line_width,
             label=r'$g_{v}(q)[2,2]$')
    #plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 0, 0], 'c--', linewidth=2, label=r'Other $g_{\omega}(q)$')
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 0, 1], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 0, 2], 'c--', linewidth=2)
    #plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 0, 4], 'c--', linewidth=2)
    #plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 0, 5], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 1, 0], 'c--', linewidth=2)
    #plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 1, 1], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 1, 2], 'c--', linewidth=2)
    #plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 1, 3], 'c--', linewidth=2)
    #plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 1, 5], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 2, 0], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 2, 1], 'c--', linewidth=2)
    #plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 2, 2], 'c--', linewidth=2)
    #plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 2, 3], 'c--', linewidth=2)
    #plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 2, 4], 'c--', linewidth=2)
    plt.xlabel("$t(s)$", fontsize=fontsize_ticks)
    plt.xticks(fontsize=fontsize_ticks)
    plt.yticks(fontsize=fontsize_ticks)
    plt.legend(fontsize=fontsize)
    ...
    # Plot g_omega(q)
    fig = plt.figure(figsize=figsize)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 3, 3], 'r--', linewidth=line_width,
             label=r'$g_{\omega}(q)[3,3]$')
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 4, 4], 'g--', linewidth=line_width,
             label=r'$g_{\omega}(q)[4,4]$')
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 5, 5], 'b--', linewidth=line_width,
             label=r'$g_{\omega}(q)[5,5]$')
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 3, 0], 'c--', linewidth=2, label=r'Other $g_{\omega}(q)$')
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 3, 1], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 3, 2], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 3, 4], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 3, 5], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 4, 0], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 4, 1], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 4, 2], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 4, 3], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 4, 5], 'c--', linewidth=2)
    ...
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 5, 3], 'c--', linewidth=2)
    plt.plot(t_eval, g_q.detach().cpu().numpy()[:, 5, 4], 'c--', linewidth=2)
    plt.xlabel("$t(s)$", fontsize=fontsize_ticks)
    plt.xticks(fontsize=fontsize_ticks)
    plt.yticks(fontsize=fontsize_ticks)
    plt.legend(fontsize=fontsize)
